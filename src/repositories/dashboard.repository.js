...
exports.getDashboard = async (req, res, next) => {
    ...
    try {
        ...
        if (vToken.status === 401) { return res.status(401).send({ "error": 401, "message": vToken.message }) }
            ...
            const productionsRes = await db.query("SELECT P.uuid, P.name, P.price, U.name AS createby, P.createdate, R.name AS modifyby, P.modifydate FROM production P LEFT JOIN users U ON P.createby=CAST(U.uuid AS VARCHAR) LEFT JOIN users R ON P.modifyby=CAST(R.uuid AS VARCHAR) WHERE CAST(P.price AS FLOAT) > 0 ORDER BY P.name;");
